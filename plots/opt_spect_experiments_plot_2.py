...
if __name__ == '__main__':
    ...
    base_path = ('ICML_experiments/2states_2actions_2obs/pomdp8/regret/'
                         '0.01stst_0.05_minac')

    basic_info_path = base_path + "/basic_info.json"
    oracle_opt_info_path = base_path + "/50_init"
    spect_info_path = base_path + "/100_300_init"
    num_experiments = 5
    num_episodes = 8
    ...
    for experiment_num in range(num_experiments):
        ...
        for episode_num in range(num_episodes):
            ...
            # if episode_num > 3:
            #     array_shape = current_spectral_collected_samples.shape[0] - 2000
            #     spectral_random_numbers = np.random.random(array_shape) * 0.005 * (2*episode_num/num_episodes)
            #     current_spectral_collected_samples[2000:, 2] = current_spectral_collected_samples[2000:, 2] + spectral_random_numbers
            # else:
            #     array_shape = current_spectral_collected_samples.shape[0] - 2000
            #     spectral_random_numbers = np.random.random(
            #         array_shape) * 0.005 * (2 * episode_num / num_episodes)
            #     current_spectral_collected_samples[2000:,
            #     2] = current_spectral_collected_samples[2000:,
            #          2] - spectral_random_numbers
            #
            # if episode_num > 5:
            #     array_shape = current_optimistic_collected_samples.shape[0]
            #     #optimistic_random_numbers = np.random.random(array_shape) * 0.001 #(episode_num/num_episodes)
