...
class POMDPSimulation:
    ...
    #         state_observation_matrix = self.pomdp.state_action_observation_matrix[:, action, :]
    ...
    #
    #     min_svd = compute_min_svd(observation_state_big_matrix)
    ...
    #             first_obs_mat = self.pomdp.state_action_observation_matrix[:, first_action, :]
    #             second_obs_mat = self.pomdp.state_action_observation_matrix[:, second_action, :]
    #
    ...
    # def compute_overcomplete_reference_matrix(self):
    #     original_row_dim = self.num_actions**2 * self.num_obs**2
    #     row_dim = self.num_actions ** 2 * self.num_obs ** 3
    #     col_dim = self.num_actions**2 * self.num_states**2
    #     overcomplete_reference_matrix = np.zeros((row_dim, col_dim))
    #
    #     random_multipliers = np.random.dirichlet([1/self.num_obs]*self.num_obs, 1).reshape(-1)
    #
    #     for obs in range(self.num_obs):
    #         overcomplete_reference_matrix[obs*original_row_dim:(obs+1)*original_row_dim, :] = (
    #                 self.reference_matrix * random_multipliers[obs])
    #
    #     min_svd = compute_min_svd(overcomplete_reference_matrix)
    #     print(f"min svd of reference matrix is {min_svd}")
