...
class WeightedMeanStrategy:
    ...
    def run(self, num_samples_to_discard, num_samples_per_estimate, num_estimates, initial_state):
        ...
        self.full_obs_count_vec = np.zeros(shape=(num_estimates, self.num_states, self.num_states,
                                    self.num_actions, self.num_actions,
                                    self.num_obs, self.num_obs))

        self.part_obs_count_vec = np.zeros(shape=(num_estimates, self.num_actions, self.num_actions,
                                    self.num_obs, self.num_obs))

        self.action_state_dist = np.zeros(shape=(num_estimates, self.num_actions, self.num_actions,
        ...
        for estim in range(num_estimates):
            ...
            self.num_samples_per_estimate[estim] = num_samples_per_estimate
            ...
            for i in range(num_samples_to_discard+num_samples_per_estimate):
                ...
                if i >= num_samples_to_discard:
                    self.full_obs_count_vec[estim, first_state, second_state, first_action, second_action, first_obs, second_obs] += 1
                    self.part_obs_count_vec[estim, first_action, second_action, first_obs, second_obs] += 1

                first_state = next_state


            estimated_trans_matrix = self.estimate_transition_matrix(estimate_num=estim)
    ...
    def estimate_transition_matrix(self, estimate_num):

        count_vector = self.part_obs_count_vec[estimate_num]
        ...
        self.action_state_dist[estimate_num] = act_stat_pair_dist

        # compute the weighted mean of the estimates
        total_num_samples = self.num_samples_per_estimate[:estimate_num+1].sum()
        weights_of_estimates = self.num_samples_per_estimate / total_num_samples
        resulting_act_state_dist = self.action_state_dist[:estimate_num+1] * weights_of_estimates[:estimate_num+1, None, None, None, None]
        resulting_act_state_dist = resulting_act_state_dist.sum(axis=0)

        reduced_act_stat_pair_dist = resulting_act_state_dist.sum(axis=1)
