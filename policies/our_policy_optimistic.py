...
class OurPolicyOptimistic:
    ...
    def choose_arm(self):
        if self.t < len(self.flatten_combinations_of_pairs_of_arms):
            return self.flatten_combinations_of_pairs_of_arms[self.t]
    ...
    def estimate_transition_matrix(self):
        first_arm, first_reward = self.action_reward_list[self.t-2]
        second_arm, second_reward = self.action_reward_list[self.t-1]
        ...
        flatten_transition_distribution = np.max([flatten_transition_distribution, np.array([0 for _ in range(flatten_transition_distribution.shape[0])])])
