...
if __name__ == '__main__':
    paths_to_read_from = ['experiments_estimation_error/10states_10actions_10obs/bandit0/exp3',
                          'experiments_estimation_error_v2/5states_8actions_10obs/bandit0/exp1']

    # fig, axs = plt.plot(figsize=(20, 6))  # , sharex=True, sharey=True)
    fig, axs = plt.subplots(1, len(paths_to_read_from),
    ...
    for j, path in enumerate(os.listdir(paths_to_read_from[0])):
        if path.endswith('arm.json') and not path.endswith('1_arm.json'):
            # Opening JSON file
            f = open(paths_to_read_from[0] + '/' + path)
            ...
            axs[0].plot(x_axis, mean_estimation_errors, line_plot_colors1[j], label=f'{num_selected_arms} arms')
    ...
    f = open(paths_to_read_from[1] + '/exp_info.json')
    ...
    print(exp_info)
    for i, path in enumerate(os.listdir(paths_to_read_from[1])):

        if path.endswith('arm.json'):
            # Opening JSON file
            f = open(paths_to_read_from[1] + '/' + path)
            # returns JSON object as
            # a dictionary
            data = json.load(f)
